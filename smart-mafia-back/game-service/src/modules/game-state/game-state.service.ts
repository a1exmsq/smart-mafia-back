...
export class GameStateService {
  ...
  async advancePhase(roomId: string): Promise<GameState> {
    ...
    if (winner) {
      ...
      void this.gateway.broadcastAutomatedNarration(roomId, {
        event: 'game_over',
        round: nextRound,
        context: winner === 'MAFIA'
          ? 'The Mafia outnumber the town and take control of the game.'
          : 'The town has rooted out every Mafia member and survives.',
        playerNames: (finalRoles ?? [])
      ...
      const narrationEvent = this.getNarrationEventForPhase(nextPhase);
      if (narrationEvent) {
        void this.gateway.broadcastAutomatedNarration(roomId, {
          event: narrationEvent,
          round: nextRound,
          context: this.buildPhaseNarrationContext(nextPhase, nightResult),
          playerNames: alivePlayers.map((player) => player.user.username),
        });
      }
    }
  ...
  async resolveVotes(roomId: string): Promise<{ eliminated: string | null; state: GameState }> {
    ...
    if (Object.keys(tally).length === 0) {
      ...
      void this.gateway.broadcastAutomatedNarration(roomId, {
        event: 'custom',
        context: 'The town hesitates, no majority forms, and nobody is eliminated this round.',
      });
    ...
    if (topTargets.length === 1) {
      ...
      void this.gateway.broadcastAutomatedNarration(roomId, {
        ...
        context: `${player.user.username} was eliminated by the town vote.`,
      });
      const newState = await this.advancePhase(roomId);
    ...
    if (snapshot.isRunoff) {
      ...
      void this.gateway.broadcastAutomatedNarration(roomId, {
        event: 'player_eliminated',
        context: `After a deadlock, fate chose ${player.user.username} for elimination.`,
      });
    ...
    void this.gateway.broadcastAutomatedNarration(roomId, {
      event: 'custom',
      context: `The vote is tied between ${tiedNames}. The town must settle it in a runoff.`,
    });
  ...
  private getNarrationEventForPhase(
    phase: GamePhase,
  ): 'day_phase' | 'voting_phase' | 'night_phase' | null {
    if (phase === GamePhase.DAY) return 'day_phase';
    if (phase === GamePhase.VOTING) return 'voting_phase';
    if (phase === GamePhase.NIGHT) return 'night_phase';
    return null;
  }

  private buildPhaseNarrationContext(
    phase: GamePhase,
    nightResult: GameSnapshot['lastNightResult'],
  ) {
    if (phase === GamePhase.DAY) {
      if (!nightResult) return 'The town opens its eyes to another uneasy morning.';
      if (nightResult.killedName) {
        return `${nightResult.killedName} did not survive the night. The town must piece together what happened.`;
      }
      if (nightResult.savedByDoctor) {
        return 'Someone was attacked in the night, but the Doctor kept them alive.';
      }
      return 'The night passed without a body, but suspicion only deepens.';
    }

    if (phase === GamePhase.VOTING) {
      return 'The debate is over. Every glance turns into judgment as the town prepares to vote.';
    }

    if (phase === GamePhase.NIGHT) {
      return 'Doors close, voices fade, and the special roles move under cover of darkness.';
    }
